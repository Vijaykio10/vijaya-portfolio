      ...
      <div className="container-max w-full">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-16 py-16">
          <div className="flex-1 max-w-2xl">
            ...
            >
              <span className="section-tag">
                <span
                  ...
                  style={{ background: '#22c55e', boxShadow: '0 0 8px #22c55e', animation: 'pulse 2s ease-in-out infinite' }}
                />
                Open to AI/ML &amp; GenAI Roles · South India
              </span>
            </motion.div>
            ...
            >
              <p className="font-mono text-sm mb-2" style={{ color: 'var(--cyan)', letterSpacing: '0.15em' }}>
                &gt; Hello, I am
              </p>
              <h1
                className="mb-3"
              ...
              >
                Vijaya <span className="gradient-text">Ragunath</span>{' '}
            ...
            >
              <span className="font-mono text-xl font-medium" style={{ color: 'var(--blue)' }}>
                {role}
                <span
                  style={{
            ...
            <motion.p
              ...
              style={{ color: 'var(--text-secondary)', maxWidth: '560px', fontFamily: 'DM Sans, sans-serif' }}
            >
              M.Sc. Applied Data Science professional building{' '}
              <span style={{ color: 'var(--text-primary)', fontWeight: 500 }}>practical AI systems</span>{' '}
              with machine learning, LLMs, RAG, computer vision, and speech AI — from{' '}
              <span style={{ color: 'var(--cyan)', fontWeight: 500 }}>prototype to usable product</span>.
            </motion.p>
            ...
            >
              ...
              <div className="flex gap-3">
                {[
                  { Icon: Github, href: 'https://github.com/Vijaykio10', label: 'GitHub' },
                  { Icon: Linkedin, href: 'https://linkedin.com/in/vijayaragunath', label: 'LinkedIn' },
                ].map(({ Icon, href, label }) => (
                  <motion.a
                    key={label}
                    href={href}
          ...
          >
            {[110, 150, 175].map((r, i) => (
              <div
                ...
                style={{
                  ...
                  borderColor: i === 0 ? 'rgba(14,165,233,0.15)' : i === 1 ? 'rgba(168,85,247,0.1)' : 'rgba(34,211,238,0.07)',
            ...
            {orbitIcons.map(({ Icon, color, size, duration, radius, label }, i) => (
              <motion.div
                key={i}
                className="absolute"
                style={{ top: '50%', left: '50%', width: '40px', height: '40px', marginTop: '-20px', marginLeft: '-20px' }}
                animate={{ rotate: 360 }}
                transition={{ duration, repeat: Infinity, ease: 'linear', direction: i % 2 === 0 ? 'normal' : 'reverse' }}
              >
                <motion.div
                  animate={{ rotate: -360 }}
                  transition={{ duration, repeat: Infinity, ease: 'linear' }}
                  className="absolute flex flex-col items-center gap-1"
                  style={{ top: `-${radius}px`, left: '50%', transform: 'translateX(-50%)' }}
                >
                  <div
                    className="w-9 h-9 rounded-xl flex items-center justify-center"
                    style={{
                      background: `rgba(${color === '#0ea5e9' ? '14,165,233' : color === '#a855f7' ? '168,85,247' : color === '#22d3ee' ? '34,211,238' : '236,72,153'}, 0.15)`,
                      border: `1px solid ${color}33`, boxShadow: `0 0 12px ${color}44`,
                    }}
                  >
                    <Icon size={size} color={color} />
                  </div>
                  <span className="font-mono text-xs" style={{ color, opacity: 0.8, letterSpacing: '0.05em' }}>
                    {label}
                  </span>
                </motion.div>
              </motion.div>
            ...
            >
              ...
              >VR</span>
              <span className="font-mono text-xs mt-1" style={{ color: 'var(--cyan)', opacity: 0.7, letterSpacing: '0.15em' }}>
                AI/ML
              </span>
            </div>
        ...
        >
          <span className="font-mono text-xs" style={{ color: 'var(--text-muted)', letterSpacing: '0.15em' }}>
            SCROLL DOWN
          </span>
          <motion.div
            animate={{ y: [0, 8, 0] }}
          ...
          >
            <div className="w-1 h-2 rounded-full" style={{ background: 'var(--cyan)' }} />
          </motion.div>
        </motion.div>
      </div>
    </section>
